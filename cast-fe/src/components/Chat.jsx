...
class Chat extends Component {
  ...
  componentDidMount() {
    this.connectChat(
      this.props.embedded ? this.props.hash : this.props.match.params.hash
    );
  }

  componentDidUpdate(prevProps, prevState, snapshot) {
    if (
      (this.props.embedded && this.props.hash !== prevProps.hash) ||
      (!this.props.embedded &&
        this.props.match.params.hash !== prevProps.match.params.hash)
    ) {
    ...
    connection.onmessage = (message) => {
      try {
        ...
        if (json && json.type === "chat") {
          ...
          if (
            this.chatWrapper.scrollHeight -
              this.chatWrapper.scrollTop -
              this.chatWrapper.clientHeight <
            72
          )
