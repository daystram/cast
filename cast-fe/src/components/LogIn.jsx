  ...
  validate(field, value) {
    switch (field) {
      case "username":
        if (!value.trim()) {
          this.setState({
            error_username: "Please enter your username or email",
          });
          return false;
        }
    ...
    axios
      ...
      .then((response) => {
        ...
        switch (response.data.code) {
          ...
          case 404:
            this.setState({
              error_username: "Username or email not registered",
            });
            return;
          case 403:
    ...
    return (
      <Container fluid style={style.content_container}>
        ...
        {this.state.error_login && (
          <Alert variant={"danger"}>{this.state.error_login}</Alert>
        )}
        {!this.state.verified && (
          <Alert variant={"warning"}>
            <Alert.Heading>Verify Your Email</Alert.Heading>
            <p>
              You need to verify your email before logging in. Check your email
        ...
        <p style={{ display: "flex", justifyContent: "space-evenly" }}>
          <Link className={"inline"} to={"/forget"}>
            Forgot password?
          </Link>
        </p>
